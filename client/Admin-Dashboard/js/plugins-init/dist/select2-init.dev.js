...
(function ($) {
  "use strict"; // single select box

  $("#single-select").select2(); // multi select box

  $('.multi-select').select2(); // dropdown option groups

  $('.dropdown-groups').select2(); //disabling options

  $('.disabling-options').select2(); //disabling a select2 control
  ...
  $(".select2-with-label-multiple").select2(); //select2 container width

  $(".select2-width-50").select2();
  $(".select2-width-75").select2(); //select2 themes
  ...
  function matchCustom(params, data) {
    ...
    if (data.text.indexOf(params.term) > -1) {
      ...
      modifiedData.text += ' (matched)'; // You can return modified objects from here
  ...
  $(".add-new-options").append(newOption).trigger('change').select2(); // create if not exists
  ...
  if ($('.create-if-not-exists').find("option[value='" + data.id + "']").length) {
    ...
    var newOption = new Option(data.text, data.id, true, true); // Append it to the select
